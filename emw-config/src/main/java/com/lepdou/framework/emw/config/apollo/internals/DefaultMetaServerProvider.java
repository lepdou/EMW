...
public class DefaultMetaServerProvider implements MetaServerProvider {
    ...
    private static final Logger logger = LoggerFactory.getLogger(
            com.lepdou.framework.emw.config.apollo.internals.DefaultMetaServerProvider.class);
    ...
    private String initMetaServerAddress() {
        ...
        if (Strings.isNullOrEmpty(metaAddress)) {
            logger.warn(
                    "Could not find meta server address, because it is not available in neither (1) JVM system property 'apollo.meta', "
                            + "(2) OS env variable 'APOLLO_META' (3) property 'apollo.meta' from server.properties nor (4) property "
                            + "'apollo.meta' from app.properties");
        } else {
